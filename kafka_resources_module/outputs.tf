output "resource-ids" {
  ...
  Kafka Cluster ID: ${var.kafka_cluster_id}
  ...
  $ confluent kafka topic produce ${confluent_kafka_topic.orders.topic_name} --environment ${var.environment_id} --cluster ${var.kafka_cluster_id} --api-key "${confluent_api_key.app-producer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-producer-kafka-api-key.secret}"
  ...
  $ confluent kafka topic consume ${confluent_kafka_topic.orders.topic_name} --from-beginning --environment ${var.environment_id} --cluster ${var.kafka_cluster_id} --api-key "${confluent_api_key.app-consumer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-consumer-kafka-api-key.secret}"
